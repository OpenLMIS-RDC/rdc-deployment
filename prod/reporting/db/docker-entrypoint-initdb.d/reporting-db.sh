...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "open_lmis_reporting" < /docker-entrypoint-initdb.d/templates/OlmisCreateAnalyticAggregateSchemaStatements.sql
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "open_lmis_reporting" < /docker-entrypoint-initdb.d/templates/OlmisInitDailyAnalyticDataStatements.sql
